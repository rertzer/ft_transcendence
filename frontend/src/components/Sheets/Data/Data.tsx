...
export function Data(props: {sx: number, sy: number, zoom: number}) {
  ...
  const [listBy, setListBy] = useState("Games Won");
  const auth = useLogin();
...
function sortUsers(users: User[], by: string) {
  ...
  return (
    <div key={"contact"}>
      ...
      <div onClick={()=>{setListBy("Games Played")}}><CreateStyledCell
      coordX={3} coordY={3} width={1} height={1} scroll_x={props.sx} scroll_y={props.sy} zoom={props.zoom}
      text={"Games Played"} fontSize={12} className={"title_data"} /></div>
      <div onClick={()=>{setListBy("Games Won")}}><CreateStyledCell
      coordX={3} coordY={4} width={1} height={1} scroll_x={props.sx} scroll_y={props.sy} zoom={props.zoom}
      text={"Games Won"} fontSize={12} className={"title_data"} /></div>
      <CreateStyledCell
