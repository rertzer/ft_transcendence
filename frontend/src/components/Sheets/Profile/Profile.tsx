  ...
  return (
    <div key={props.index}>
      ...
      <CreateStyledCell
        ...
        width={1}
        height={1}
        text={props.game.opponentUserName.toString()}
        fontSize={12}
        className={"dataItemButton"}
        onClick={() => navigate(`/profile/${props.game.opponentLogin}`)}
      />
      <CreateStyledCell
        coordX={props.index}
        coordY={5}
        width={1}
        height={1}
        text={props.game.myScore?.toString() || ""}
        fontSize={12}
        className={"dataItem"}
      />
      <CreateStyledCell
        ...
        width={1}
        height={1}
        text={props.game.myOpponentScore?.toString() || ""}
        fontSize={12}
        className={"dataItem"}
      />
      <CreateStyledCell
        ...
        width={1}
        height={1}
        text={props.game.date_begin.toString().slice(0, 10)}
        fontSize={11}
        className={"dataItem"}
      />
      <CreateStyledCell
        ...
        width={1}
        height={1}
        text={Math.floor(props.game.durationInSec || 0)?.toString() + "s"}
        fontSize={12}
        className={"dataItem"}
      />
    </div>
...
function Profile() {
  ...
  const fetchImage = async () => {
    try {
      ...
      const res = await fetch(
        `http://${process.env.REACT_APP_URL_MACHINE}:4000/user/avatar/${user.avatar}`,
        {
          method: "GET",
          headers: { Authorization: bearer },
        }
      );
      console.log("fetchImage on route /user/avatar/", user.avatar, res.status);
      if (res.status === 200) {
  ...
  const fetchGameUser = async () => {
    try {
      ...
      const data = await fetch(
        `http://${process.env.REACT_APP_URL_MACHINE}:4000/gameStats/login/${user.login}`,
        {
          method: "GET",
          headers: { Authorization: bearer },
        }
  ...
  const fetchUser = async (login: string) => {
    try {
      ...
      const data = await fetch(
        `http://${process.env.REACT_APP_URL_MACHINE}:4000/user/` + login,
        {
          method: "GET",
          headers: { Authorization: bearer },
        }
      );
      console.log("fetchuser status", data.status);
      if (data.status === 200) {
        const newUser = await data.json();
        setUser(newUser);
      } else {
  ...
  function calculate_edit_Y() {
    const result = Math.floor(
      (windowWidthRef.current - ((windowWidthRef.current / 100) * 2 + 31)) /
        (80 + (zoom - 100) / 2) -
        1
    );
  ...
  async function addToFriends(login: string) {
    ...
    try {
      await fetch(
        `http://${process.env.REACT_APP_URL_MACHINE}:4000/friend/addFriend/`,
        requestOptions
      );
    } catch (error) {
    ...
    if (chat === "none") {
      ...
      try {
        const response = await fetch(
          `http://${process.env.REACT_APP_URL_MACHINE}:4000/chatOption/newPrivateConv`,
          requestOptions
        );
        if (!response.ok) {
  ...
  return (
    ...
    >
      ...
      <CreateStyledCell
        ...
        width={1}
        height={1}
        text={`${gameUser.numberGamesBasic}`}
        className={"data_profile"}
        fontSize={12}
      />
      <CreateStyledCell
        ...
        width={1}
        height={1}
        text={`${gameUser.numberGamesAdvanced}`}
        className={"data_profile"}
        fontSize={12}
      />
      <CreateStyledCell
      ...
      <CreateStyledCell
        ...
        width={1}
        height={1}
        text={`${gameUser.numberWonAdvanced}`}
        className={"data_profile"}
        fontSize={12}
      />
      <CreateStyledCell
      ...
      <CreateStyledCell
        ...
        width={1}
        height={1}
        text={`${gameUser.numberLostAdvanced}`}
        className={"data_profile"}
        fontSize={12}
      />
      <CreateStyledCell
      ...
      <CreateStyledCell
        ...
        width={1}
        height={1}
        text={`${Math.floor(gameUser.totalGameDurationBasicInSec)}s`}
        className={"data_profile"}
        fontSize={12}
      />
      <CreateStyledCell
        ...
        width={1}
        height={1}
        text={`${Math.floor(gameUser.totalGameDurationAdvancedInSec)}s`}
        className={"data_profile"}
        fontSize={12}
      />
      <CreateStyledCell
        ...
        width={1}
        height={1}
        text={`${Math.floor(gameUser.totalGameDurationInSec)}s`}
        className={"data_profile"}
        fontSize={12}
      />
      <CreateStyledCell
      ...
      <CreateStyledCell
        ...
        coordY={1}
        width={8}
        height={sizeOfList == 0 ? 2 : sizeOfList + 1}
        text={""}
        className={"border"}
        fontSize={12}
      />
      ...
      {!isAuth() && (
        <div>
          ...
          <CreateStyledCell
            ...
            className={"edit_profile"}
            fontSize={12}
            onClick={() => addToFriends(user.login)}
          />
        </div>
      )}
    </div>
