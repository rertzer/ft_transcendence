...
export class MyGateway {
  ...
  handleConnection(client: Socket) {
    ...
    client.on("disconnect", () => {
      ...
      this.socketsLogin = this.socketsLogin.filter((s) => {
        return s.sock !== client;
      });
    });
  }
  ...
  async onNewMessage(
    ...
    if (
      !this.mutedUserService.IsMutedUser(
        messageData.login,
        messageData.idOfChat
      )
    ) {
      ...
      if (targetSocket !== undefined) {
        ...
        await this.prismaChatService.addChatMessage(
          messageData.idOfChat,
          targetSocket.idOfLogin,
          messageData.content,
          getDate(),
          messageData.serviceMessage
        );
        this.server
          .to(messageData.idOfChat.toString())
  ...
  async onSendMessage(
    @MessageBody()
    messageData: { msg: string; loginToSend: string; idOfUser: string },
    @ConnectedSocket() client: Socket
  ) {
    const targetSocket = this.socketsLogin.find(
      (socket) => socket.sock === client
    ...
    if (targetSocket !== undefined) {
      const userExist = await this.prismaChatService.checkLogin(
        messageData.loginToSend
      );
      if (userExist === false) {
        ...
        const idToSend = await this.prismaChatService.getIdOfLogin(
          messageData.loginToSend
        );
        if (idToSend !== undefined)
          this.prismaChatService.addPrivateMessage(
            targetSocket.idOfLogin,
            idToSend,
            messageData.msg
          );
  ...
  async onNewPrivateConv(
    @MessageBody() messageData: { sender: string; receiver: string },
    @ConnectedSocket() client: Socket
  ) {
    const targetSocket = this.socketsLogin.find(
      (socket) => socket.sock === client
    ...
    if (targetSocket !== undefined) {
      const receiverSocket = this.socketsLogin.find(
        (socket) => socket.login === messageData.receiver
      );
      if (receiverSocket) {
        const allGood = await this.privateConv.setDirectConv(
          messageData.sender,
          targetSocket.idOfLogin,
  ...
  async onCreateChat(
    ...
    if (targetSocket !== undefined) {
      ...
      CreateRoom.createChat(
        messageData.login,
        targetSocket.idOfLogin,
        messageData.chatPassword,
  ...
  async onRetrieveMessage(
    @MessageBody() messageData: { numberMsgToDisplay: number; chatId: number },
    @ConnectedSocket() client: Socket
  ) {
    const targetSocket = this.socketsLogin.find(
      (socket) => socket.sock === client
  ...
  mutedUser(
    @MessageBody() user: { login: string; chatId: number; time: number },
    @ConnectedSocket() client: Socket
  ) {
