...
export class JoinChatService {
  ...
  async joinChat(
    ...
    if (!(await this.prismaService.checkIfUserIsBanned(chat_id, loginId))) {
      const added = await this.addUserToChat(
        loginId,
        chat_id,
        user_role,
        password
      ...
      return value;
    } else return -2; //pour indiquer au front que le user est ban du chat qu'il essaie de rejoindre
  }
  ...
  async addUserToChat(
    ...
    const getPasswordOfChat =
      await this.prismaService.getPasswordOfChat(chat_id);
    let pwMatches = false;
    ...
    if (password !== null && getPasswordOfChat != undefined) {
      ...
      pwMatches = await argon.verify(getPasswordOfChat, password);
    } else {
      // hashed_password = password;
    }

    if (password == null || pwMatches) {
      (await this.prismaService.userHasbeenKickedInChat(loginId, chat_id)) ==
        true; //user updated to removed kicked value
      const chatId = await this.prismaService.addChanelUser(
        chat_id,
        loginId,
        user_role,
        getDate(),
      ...
      if (chatId !== undefined) return chatId.toString();
    }
    return undefined;
  }
}
