...
export class addFriendController {
  ...
  async addFriend(@Body() data: { login: string; friendToAdd: string }) {
    ...
    const targetSocket = SockArray.find(
      (socket) => socket.login === data.login
    );
    if (targetSocket) {
      const done = await this.prismaFriendService.addFriend(
  ...
  async checkIfFriend(
    ...
    if (idLog && idFriend) {
      const friend = await this.prismaFriendService.alreadyFriend(
        idLog,
        idFriend
      );
  ...
  async deleteFriend(
    ...
    const succeed = await this.prismaFriendService.deleteFriend(
      idUser,
      idFriend
    );
    if (succeed) {
  ...
  async listFriend(@Param("login") login: string) {
    const listFriends = [];
    let connected;
    ...
    if (list) {
      ...
      for (const element of list) {
        const getLoginOfId = await this.prismaFriendService.getLoginOfId(
          element.friend_id
        );
        if (getLoginOfId) {
          const connect = SockArray.find(
            (item) => item.idOfLogin === element.friend_id
          );
          if (connect) {
            const inGame = await this.prismaFriendService.checkInGame(
              getLoginOfId.id
            );
            if (inGame) connected = "in Game";
            else connected = "online";
          } else connected = "offline";
          const friend = {
